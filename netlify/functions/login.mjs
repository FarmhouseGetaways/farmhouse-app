 ...
 *
 * The door into this app's own small admin corner (see CLAUDE.md — test
 * push, subscriber count, app-level settings; the guest broadcast lives on
 * farmhouse-admin now). Same Authorization Code flow as farmhouse-admin's
 * own login.mjs — a plain top-level redirect to Google's sign-in page, not
 * Google's rendered button, because that button doesn't work in Safari
 * (confirmed live, see farmhouse-admin's CLAUDE.md for the detail).
 *
 ...
 * this function's own URL are in that client's Authorized JavaScript
 * origins / Authorized redirect URIs in Google Cloud Console.
 */
...
export default async (req) => {
  ...
  if (req.method === "GET") {
    if (url.searchParams.get("code") || url.searchParams.get("error")) {
      ...
      // The browser sometimes fires this redirect twice for one sign-in
      // (seen live 10 Sep 2026, both requests carrying the same code).
      // Google's auth code is single-use, so the second exchange always
      // fails — but if the first one already landed and set a valid
      // session, treat the duplicate as success instead of bouncing to an
      // error page.
      if (currentEmail(req)) {
