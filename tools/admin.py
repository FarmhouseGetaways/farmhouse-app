...
<div id="panel" hidden>
  ...
  <div class="wrap">
    ...
    <!-- ---------------------------------------------------------------- -->
    <section class="pane" id="pane-checkout">
      <div class="sec">
        <p class="eyebrow">Mini Barn Market</p>
        <h1 class="big"><span id="co-revenue">&mdash;</span>
          <span class="unit">taken this day</span></h1>
        <label class="fine" for="co-date">Date</label>
        <input id="co-date" type="date" style="{FIELD}">
        <div class="stat-grid" id="co-stats"></div>

        <h2 class="mid" style="margin-top:1.4rem">What was missed</h2>
        <p class="fine">Scans that didn't confidently match the catalog &mdash;
          a photo is kept for these only, so this is also the whole reason one
          got kept.</p>
        <div id="co-misses"></div>

        <p class="fine" id="co-note">Loading&hellip;</p>
      </div>
    </section>
...
ADMIN_CSS = """<style>
  ...
  .stat-grid {
    ...
    gap: .5rem; margin: .8rem 0 0;
  }
  .stat-tile {
    border: 1px solid var(--line); border-radius: var(--radius);
    background: var(--night-2); padding: .7rem .8rem;
  }
  .stat-tile b { display: block; font-size: 1.3rem; line-height: 1.1; }
  .stat-tile span {
    font-size: .58rem; font-weight: 700; letter-spacing: .1em;
    text-transform: uppercase; color: var(--mute-2);
  }
  .stat-tile.warn b { color: var(--mbm-red); }
  .stat-tile.good b { color: var(--fstv); }
  .miss-row {
  ...
  .miss-thumb {
    ...
    background: var(--night-2); flex: none;
  }
  .miss-meta { flex: 1; min-width: 0; font-size: .82rem; }
  .miss-meta .t {
    font-size: .6rem; color: var(--mute-2); text-transform: uppercase; letter-spacing: .06em;
  }
  .miss-meta .said { color: var(--mute); font-style: italic; }
</style>"""
...
(function () {
  ...
  /* ---- checkout activity (Mini Barn Market self-checkout, a separate
     site — see checkout-activity.mjs) ---- */
  ...
  function statTile(label, value, kind) {
    return '<div class="stat-tile' + (kind ? " " + kind : "") + '"><b>' + esc(value) +
           '</b><span>' + esc(label) + '</span></div>';
  }

  function missRow(e) {
    var thumb = e.photoDataUrl
      ? '<img class="miss-thumb" src="' + e.photoDataUrl + '" alt="">'
      : '<div class="miss-thumb"></div>';
    return '<div class="miss-row">' + thumb +
      '<div class="miss-meta"><div class="t">' + esc(timeStr(e.ts)) + ' &middot; confidence: ' + esc(e.confidence) + '</div>' +
      '<div>' + esc(e.proposedName || "(nothing proposed)") + '</div>' +
      '<div class="said">' + (e.spokenText ? "Said: \\u201c" + esc(e.spokenText) + "\\u201d" : "(nothing said)") + '</div>' +
      '</div></div>';
  }

  async function loadCheckout() {
    var dateVal = document.getElementById("co-date").value;
    note("co-note", "Loading\\u2026");
    try {
      var res = await api("checkout-activity" + (dateVal ? "?date=" + encodeURIComponent(dateVal) : ""));
      var d = await res.json();
      if (!d.ok) { note("co-note", d.error || "Could not load checkout activity."); return; }
      if (!document.getElementById("co-date").value) document.getElementById("co-date").value = d.date;

      var s = d.summary || {};
      document.getElementById("co-revenue").textContent = money(s.revenueCents);
      document.getElementById("co-stats").innerHTML =
        ...
        statTile("Charges failed", s.chargesFailed, s.chargesFailed ? "warn" : "");

      var misses = (d.events || []).filter(function (e) { return e.type === "scan" && !e.matchedProductId; });
      document.getElementById("co-misses").innerHTML = misses.length
        ? misses.map(missRow).join("")
        : '<p class="fine">Nothing missed this day.</p>';

      note("co-note", (d.events || []).length ? (d.events.length + " events logged this day.") : "Nothing logged this day.");
    } catch (err) {
      note("co-note", "Could not reach the checkout site.");
    }
  }
